...
#include "TestTransform2.h"

namespace {
  const char Name[] = "test_transform2";
  const char Desc[] = "Exercise ew::Transform2.\n";
...
// Deliberately not const.
  ...
  ewtst::TestTransform2 Tst;
}

void
ewtst::TestTransform2::random_transform(ew::Transform2 *t)
{
...
void
ewtst::TestTransform2::check_apply(const ew::Transform2 *tr, double *d,
 const double *de)
{
  float f[DIM10];
  ...
  for (int i = 0; i < DIM10; i += 1) {
    if (std::fabs(de[i] - da[i]) > 1e-5 || std::fabs(de[i] - fa[i]) > 1e-5) {
      report_error("apply");
      break;
    }
  }
...
bool
ewtst::TestTransform2::transforms_differ(const ew::Transform2 *tr,
 const ew::Transform2 *te) const
{
  for (int i = 0; i < DIM; i += 1) {
...
double
ewtst::TestTransform2::determinant(const ew::Transform2 *tr) const
{
  return tr->orthog[0][0] * tr->orthog[1][1] -
   tr->orthog[0][1] * tr->orthog[1][0];
}

const char *
ewtst::TestTransform2::test01(bool return_name)
{
  if (return_name) {
    return "translate";
  }
...
const char *
ewtst::TestTransform2::test02(bool return_name)
{
  if (return_name) {
    return "scale";
  }
...
const char *
ewtst::TestTransform2::test03(bool return_name)
{
  if (return_name) {
    return "rotate";
  }
...
const char *
ewtst::TestTransform2::test04(bool return_name)
{
  if (return_name) {
...
const char *
ewtst::TestTransform2::test05(bool return_name)
{
  if (return_name) {
...
const char *
ewtst::TestTransform2::test06(bool return_name)
{
  if (return_name) {
...
const char *
ewtst::TestTransform2::test07(bool return_name)
{
  if (return_name) {
...
const char *
ewtst::TestTransform2::test08(bool return_name)
{
  if (return_name) {
  ...
  for (int i = 0; i < DIM10; i += DIM) {
    ...
    tr.apply(da, d + i);
    double b[4];
    b[0] = aa[0] * d[i] + aa[4] * d[i + 1] + aa[8] * 999.9 + aa[12] * 1.0;
    b[1] = aa[1] * d[i] + aa[5] * d[i + 1] + aa[9] * 999.9 + aa[13] * 1.0;
    b[2] = aa[2] * d[i] + aa[6] * d[i + 1] + aa[10] * 999.9 + aa[14] * 1.0;
    b[3] = aa[3] * d[i] + aa[7] * d[i + 1] + aa[11] * 999.9 + aa[15] * 1.0;
    if (std::fabs(b[0] - da[0]) > 1e-5 ||
     std::fabs(b[1] - da[1]) > 1e-5 ||
     std::fabs(b[2] - 999.9) > 1e-5 ||
...
const char *
ewtst::TestTransform2::test09(bool return_name)
{
  if (return_name) {
    return "get_valid";
  }
...
const char *
ewtst::TestTransform2::test10(bool return_name)
{
  if (return_name) {
...
const char *
ewtst::TestTransform2::test11(bool return_name)
{
  if (return_name) {
...
const char *
ewtst::TestTransform2::test12(bool return_name)
{
  if (return_name) {
...
const char *
ewtst::TestTransform2::test(int n, bool return_name)
{
  switch(n) {
  case 1:
...
ewtst::TestTransform2::TestTransform2() :
 Test(Name, Desc, DebugVal)
{
}

int
ewtst::TestTransform2::exec(int ac, char **av)
{
