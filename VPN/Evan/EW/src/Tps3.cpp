...
// This is the number of affine components of a D-valued spline.
  ...
  const char ErrorAxesEigenanalysis[] =
   ...
   " (the eigenanalysis failed to converge).";
// Represent anti-symmetric matrices with one 1 and one -1 as:
//   {row containing 1, row containing -1, row of all 0s}.
// Generate 2 independent vectors orthogonal to the input.
// These aren't necessarily the same length as the input, or orthogonal to each
// other.
  const int Orthog[3][2][3] = {
    {{0, 1, 2}, {0, 2, 1}},
    {{0, 1, 2}, {1, 2, 0}},
    {{0, 2, 1}, {1, 2, 0}}
  };
  ...
  inline int
  MaxElement(const double *v)
  {
    int i = 0;
    double max = std::fabs(v[0]);
    double f = std::fabs(v[1]);
    if (max < f) {
      i = 1;
      max = f;
    }
  ...
  inline void
  GetPerps(double perp1[D], double perp2[D], const double inp[D])
  {
    int maxi = MaxElement(inp);
    perp1[Orthog[maxi][0][0]] = inp[Orthog[maxi][0][1]];
    perp1[Orthog[maxi][0][1]] = -inp[Orthog[maxi][0][0]];
    perp1[Orthog[maxi][0][2]] = 0.0;
    perp2[Orthog[maxi][1][0]] = inp[Orthog[maxi][1][1]];
    perp2[Orthog[maxi][1][1]] = -inp[Orthog[maxi][1][0]];
    perp2[Orthog[maxi][1][2]] = 0.0;
  }
...
/// of landmarks.
/// @param[out] L
///   This is a matrix of size (@a n_lmks + 4) x (@a n_lmks + 4).
///   The result is a symmetric and only the upper diagonal part is stored.
/// @param[in] lmks
///   This is a matrix of size @a n_lmks x 3 containing the landmark
///   coordinates.
/// @param[in] n_lmks
///   This is the number of landmarks.

void
ew::Tps3::interaction(double *L, const double *lmks, int n_lmks)
{
  int k;
// Constant control
  k = 0;
...
///   If @a is_mixed is @c false, it is at most @a n_lmks + 4.
///   Otherwise it is less than 3 * (@a n_lmks + 4).
///   If there are no semi-landmarks, it is exactly @a n_lmks + 4.
///   In general, the difference between these upper limits and @a f_size is
///   the sum of the elements of @a relax_dims.
/// @param[out] is_mixed
///   This is the location to store a @c boolean value indicating whether the 3
...
void
ew::Tps3::algebraic_size(int *f_size, bool *is_mixed, bool *is_reduced,
...
///   coordinates, or when the landmarks are coplanar.
...
///   This is a matrix of size @a n_lmks x 3.
///   For every semi-landmark sliding along a line, the corresponding row of
///   this matrix should be a unit vector in the direction of the line.
///   For every semi-landmark sliding along a plane, the corresponding row of
///   this matrix should be a unit vector normal to the plane.
///   The other rows need not be set.
///   If there are no semi-landmarks of dimension 1 or 2, this can be @c NULL.
/// @param[in] f_size
///   See #algebraic_size.
/// @param[in] is_mixed
///   See #algebraic_size.
...
void
ew::Tps3::factorize(double *F, int *pivots, double *nonsingularity,
...
// upper diagonal parts.
    ...
    for (int i = 0; i < n_lmks + D1; i += 1) {
      ...
      case RELAX_DIM_0:
        ...
        for (int j = i; j < n_lmks + D1; j += 1) {
          ...
          case RELAX_DIM_0:
            ...
            break;
          case RELAX_DIM_1:
            GetPerps(perpj1, perpj2, relax_params_opt + D * (j - D1));
            for (int x = 0; x < D; x += 1) {
              F[(a + x) * f_size + b] =
               L[i * (n_lmks + D1) + j] * perpj1[x];
              F[(a + x) * f_size + b + 1] =
               L[i * (n_lmks + D1) + j] * perpj2[x];
            }
            b += 2;
            break;
          case RELAX_DIM_2:
      ...
      case RELAX_DIM_1:
        b = a;
        GetPerps(perpi1, perpi2, relax_params_opt + D * (i - D1));
        for (int j = i; j < n_lmks + D1; j += 1) {
          double perpj1[D], perpj2[D];
          int rdoj;
          if (j < D1) {
          ...
          case RELAX_DIM_0:
            for (int y = 0; y < D; y += 1) {
              F[a * f_size + b + y] =
               perpi1[y] * L[i * (n_lmks + D1) + j];
              F[(a + 1) * f_size + b + y] =
               perpi2[y] * L[i * (n_lmks + D1) + j];
            }
            b += D;
            break;
          case RELAX_DIM_1:
            GetPerps(perpj1, perpj2, relax_params_opt + D * (j - D1));
            F[a * f_size + b] = L[i * (n_lmks + D1) + j] *
             (perpi1[0] * perpj1[0] + perpi1[1] * perpj1[1] +
             perpi1[2] * perpj1[2]);
            F[a * f_size + b + 1] = L[i * (n_lmks + D1) + j] *
             (perpi1[0] * perpj2[0] + perpi1[1] * perpj2[1] +
             perpi1[2] * perpj2[2]);
            if (i != j) {
              F[(a + 1) * f_size + b] = L[i * (n_lmks + D1) + j] *
               (perpi2[0] * perpj1[0] + perpi2[1] * perpj1[1] +
               perpi2[2] * perpj1[2]);
            }
            F[(a + 1) * f_size + b + 1] = L[i * (n_lmks + D1) + j] *
             (perpi2[0] * perpj2[0] + perpi2[1] * perpj2[1] +
             perpi2[2] * perpj2[2]);
            b += 2;
            break;
          case RELAX_DIM_2:
            F[a * f_size + b] = L[i * (n_lmks + D1) + j] * (
             perpi1[0] * relax_params_opt[(j - D1) * D] +
             perpi1[1] * relax_params_opt[(j - D1) * D + 1] +
             perpi1[2] * relax_params_opt[(j - D1) * D + 2]);
            F[(a + 1) * f_size + b] = L[i * (n_lmks + D1) + j] * (
             perpi2[0] * relax_params_opt[(j - D1) * D] +
             perpi2[1] * relax_params_opt[(j - D1) * D + 1] +
             perpi2[2] * relax_params_opt[(j - D1) * D + 2]);
            b += 1;
            break;
          }
        }
      ...
      case RELAX_DIM_2:
        ...
        for (int j = i; j < n_lmks + D1; j += 1) {
          ...
          case RELAX_DIM_0:
            ...
            break;
          case RELAX_DIM_1:
            GetPerps(perp1, perp2, relax_params_opt + D * (j - D1));
            F[a * f_size + b] = L[i * (n_lmks + D1) + j] * (
             relax_params_opt[(i - D1) * 3] * perp1[0] +
             relax_params_opt[(i - D1) * 3 + 1] * perp1[1] +
             relax_params_opt[(i - D1) * 3 + 2] * perp1[2]);
            F[a * f_size + b + 1] = L[i * (n_lmks + D1) + j] * (
             relax_params_opt[(i - D1) * 3] * perp2[0] +
             relax_params_opt[(i - D1) * 3 + 1] * perp2[1] +
             relax_params_opt[(i - D1) * 3 + 2] * perp2[2]);
            b += 2;
            break;
          case RELAX_DIM_2:
            F[a * f_size + b] =
             (relax_params_opt[(i - D1) * D] * relax_params_opt[(j - D1) * D] +
             relax_params_opt[(i - D1) * D + 1] * relax_params_opt[(j - D1) *
             D + 1] +
             relax_params_opt[(i - D1) * D + 2] * relax_params_opt[(j - D1) *
             D + 2]) *
             L[i * (n_lmks + D1) + j];
            b += 1;
            break;
...
/// @param[out] spline
///   This is a matrix of size (@a n_lmks + 4) x 3.
...
///   See #factorize.
/// @param[in] lmk_images
///   This is a matrix of size @a n_lmks x 3 containing the landmark image
///   coordinates.
/// @param[in] relax_dims_opt
///   See #algebraic_size.
/// @param[in] relax_params_opt
...
void
ew::Tps3::solve(double *spline, double *energy_opt,
...
// Calculate R^T * lmk_images, where R is the matrix described above.
    ...
    for (int i = 0; i < n_lmks; i += 1) {
      ...
      case RELAX_DIM_0:
        ...
        a += 1;
        scratch[a] = lmk_images[D * i + 2];
        a += 1;
        break;
      case RELAX_DIM_1:
        GetPerps(perp1, perp2, relax_params_opt + D * i);
        scratch[a] = lmk_images[D * i] * perp1[0] +
         lmk_images[D * i + 1] * perp1[1] + lmk_images[D * i + 2] * perp1[2];
        a += 1;
        scratch[a] = lmk_images[D * i] * perp2[0] +
         lmk_images[D * i + 1] * perp2[1] + lmk_images[D * i + 2] * perp2[2];
        a += 1;
        break;
      case RELAX_DIM_2:
        scratch[a] = lmk_images[D * i] * relax_params_opt[D * i] +
         lmk_images[D * i + 1] * relax_params_opt[D * i + 1] +
         lmk_images[D * i + 2] * relax_params_opt[D * i + 2];
        a += 1;
        break;
      case RELAX_DIM_D:
        break;
...
// Calculate R * solution-vector, where R is the matrix described above.
    ...
    for (int i = 0; i < n_lmks + D1; i += 1) {
      ...
      case RELAX_DIM_0:
        ...
        a += 1;
        spline[D * i + 2] = scratch[a];
        a += 1;
        break;
      case RELAX_DIM_1:
        GetPerps(perp1, perp2, relax_params_opt + D * (i - D1));
        spline[D * i] = perp1[0] * scratch[a] + perp2[0] * scratch[a + 1];
        spline[D * i + 1] = perp1[1] * scratch[a] + perp2[1] * scratch[a + 1];
        spline[D * i + 2] = perp1[2] * scratch[a] + perp2[2] * scratch[a + 1];
        a += 2;
        break;
      case RELAX_DIM_2:
        spline[D * i] = relax_params_opt[D * (i - D1)] * scratch[a];
        spline[D * i + 1] = relax_params_opt[D * (i - D1) + 1] * scratch[a];
        spline[D * i + 2] = relax_params_opt[D * (i - D1) + 2] * scratch[a];
        a += 1;
        break;
      case RELAX_DIM_D:
        spline[D * i] = spline[D * i + 1] = spline[D * i + 2] = 0;
        break;
      }
    }
  }
  ...
  if (relax_lmk_images_opt != 0) {
    for (int i = 0; i < n_lmks; i += 1) {
      if (relax_dims_opt == 0 || relax_dims_opt[i] == RELAX_DIM_0) {
        ...
        relax_lmk_images_opt[D * i + 2] = lmk_images[D * i + 2];
      } else {
...
///   This is a matrix of size @a n_points x 3.
///   It is the location to store the coordinates of the images of the points.
/// @param[in] points
///   This is a matrix of size @a n_points x 3, containing the coordinates of
///   the points
/// @param[in] n_points
 ...
 const double *spline, const double *lmks, int n_lmks)
{
  for (int i = 0; i < n_points; i += 1) {
    ...
    for (int j = 0; j < n_lmks; j += 1) {
      ...
      double dz = lmks[j * D + 2] - z;
      double r2 = dx * dx + dy * dy + dz * dz;
      double v = -std::sqrt(r2);
      sx += v * spline[j * D + DD1];
      sy += v * spline[j * D + DD1 + 1];
      sz += v * spline[j * D + DD1 + 2];
    }
    sx += spline[0] + spline[D] * x + spline[2 * D] * y + spline[3 * D] * z;
    sy += spline[1] + spline[D + 1] * x + spline[2 * D + 1] * y +
     spline[3 * D + 1] * z;
    sz += spline[2] + spline[D + 2] * x + spline[2 * D + 2] * y +
     spline[3 * D + 2] * z;
    point_images[i * D] = sx;
    point_images[i * D + 1] = sy;
    point_images[i * D + 2] = sz;
  }
}
...
///   See #map.
/// @param[in] lmk
///   This points to an array of size 3 containing the coordinates of the
///   landmark.

void
ew::Tps3::cache(float *cache, const float *points, int n_points,
 const double *lmk)
{
  for (int i = 0; i < n_points; i += 1) {
    double dx = points[i * D] - lmk[0];
    double dy = points[i * D + 1] - lmk[1];
    double dz = points[i * D + 2] - lmk[2];
    double r2 = dx * dx + dy * dy + dz * dz;
    double v = -std::sqrt(r2);
    cache[i] = v;
  }
 ...
 float *const (*caches), int n_points, const double *spline, int n_lmks)
{
  ...
  for (int j = 0; j < n_lmks; j += 1) {
    ...
    for (int i = 0; i < n_points; i += 1) {
      ...
      point_images[D * i + 2] += u[i] * spline[D * j + DD1 + 2];
    }
  }
  for (int i = 0; i < n_points; i += 1) {
    point_images[D * i] += spline[0] +
     spline[D] * points[D * i] +
     spline[2 * D] * points[D * i + 1] +
     spline[3 * D] * points[D * i + 2];
    point_images[D * i + 1] += spline[1] +
     spline[D + 1] * points[D * i] +
     spline[2 * D + 1] * points[D * i + 1] +
     spline[3 * D + 1] * points[D * i + 2];
    point_images[D * i + 2] += spline[2] +
     spline[D + 2] * points[D * i] +
     spline[2 * D + 2] * points[D * i + 1] +
     spline[3 * D + 2] * points[D * i + 2];
  }
}
...
///   stored.
///   If @a is_mixed, it has size (3 * @a n_lmks) x (3 * @a n_lmks).
...
void
ew::Tps3::bending_energy(double *B, const double *F, const int *pivots,
...
// Here B is D * n_lmks * D * n_lmks.
    ...
    for (int i = 0; i < n_lmks; i += 1) {
      ...
      case RELAX_DIM_0:
        ...
        for (int j = i; j < n_lmks; j += 1) {
          ...
          case RELAX_DIM_0:
            ...
            break;
          case RELAX_DIM_1:
            GetPerps(perpj1, perpj2, relax_params_opt + D * j);
            for (int x = 0; x < D; x += 1) {
              for (int y = 0; y < D; y += 1) {
                B[(i * D + x) * n_lmks * D + j * D + y] =
                 B[(j * D + y) * n_lmks * D + i * D + x] =
                 f_inv[(a + x) * f_size + b] * perpj1[y] +
                 f_inv[(a + x) * f_size + b + 1] * perpj2[y];
              }
            }
            b += 2;
            break;
      ...
      case RELAX_DIM_1:
        b = a;
        GetPerps(perpi1, perpi2, relax_params_opt + D * i);
        for (int j = i; j < n_lmks; j += 1) {
          double perpj1[D], perpj2[D];
          switch (relax_dims_opt[j]) {
          case RELAX_DIM_0:
            for (int x = 0; x < D; x += 1) {
              for (int y = 0; y < D; y += 1) {
                B[(i * D + x) * n_lmks * D + j * D + y] =
                 B[(j * D + y) * n_lmks * D + i * D + x] =
                 perpi1[x] * f_inv[a * f_size + (b + y)] +
                 perpi2[x] * f_inv[(a + 1) * f_size + (b + y)];
              }
            }
            b += D;
            break;
            ...
            for (int x = 0; x < D; x += 1) {
              for (int y = 0; y < D; y += 1) {
                if (b > a) {
                  B[(i * D + x) * n_lmks * D + j * D + y] =
                   B[(j * D + y) * n_lmks * D + i * D + x] =
                   perpi1[x] * perpj1[y] * f_inv[a * f_size + b] +
                   perpi1[x] * perpj2[y] * f_inv[a * f_size + b + 1] +
                   perpi2[x] * perpj1[y] * f_inv[(a + 1) * f_size + b] +
                   perpi2[x] * perpj2[y] * f_inv[(a + 1) * f_size + b + 1];
                } else {
                  B[(i * D + x) * n_lmks * D + j * D + y] =
                   B[(j * D + y) * n_lmks * D + i * D + x] =
                   perpi1[x] * perpj1[y] * f_inv[a * f_size + b] +
                   perpi1[x] * perpj2[y] * f_inv[a * f_size + b + 1] +
                   perpi2[x] * perpj1[y] * f_inv[b * f_size + a + 1] +
                   perpi2[x] * perpj2[y] * f_inv[(a + 1) * f_size + b + 1];
                }
              }
            }
          ...
          case RELAX_DIM_2:
            for (int x = 0; x < D; x += 1) {
              for (int y = 0; y < D; y += 1) {
                B[(i * D + x) * n_lmks * D + j * D + y] =
                 B[(j * D + y) * n_lmks * D + i * D + x] =
                 perpi1[x] * relax_params_opt[D * j + y] *
                 f_inv[a * f_size + b] +
                 perpi2[x] * relax_params_opt[D * j + y] *
                 f_inv[(a + 1) * f_size + b];
              }
            }
            b += 1;
          ...
          case RELAX_DIM_1:
            ...
            for (int x = 0; x < D; x += 1) {
              for (int y = 0; y < D; y += 1) {
                B[(i * D + x) * n_lmks * D + j * D + y] =
                 B[(j * D + y) * n_lmks * D + i * D + x] =
                 (f_inv[a * f_size + b] * perpj1[y] +
                 f_inv[a * f_size + b + 1] * perpj2[y]) *
                 relax_params_opt[i * D + x];
              }
            }
            b += 2;
            break;
...
void
ew::Tps3::principal_warps(double *P, double *evals, const double *B,
...
/// @param[out] centroid
///   This points to an array of size 3 to store the centroid of the
///   configuration.
/// @param[out] sum_squares
///   This points to an array of size 3 to store the sum of squared
...
void
ew::Tps3::principal_axes(double *centroid, double *sum_squares,
...
// Check orientation.
  if (axes[0] * (axes[4] * axes[8] - axes[5] * axes[7]) +
   axes[1] * (axes[5] * axes[6] - axes[3] * axes[8]) +
   axes[2] * (axes[3] * axes[7] - axes[4] * axes[6]) < 0.0) {
    for (int i = 0; i < D; i += 1) {
      axes[D * (D - 1) + i] *= -1.0;
    }
  }
...
/// If the configuration is degenerate (contained within a plane), this should
/// not be called.
/// @param[out] basis
///   This is a matrix of size 5 x @a n_lmks x 3.
///   The shape variables of the 5 basis elements are stored here.
///   The first index is the number of the basis element.
/// @param[out] transforms_opt
///   This points to an array of size 5 * 3 * 3.
///   Uniform shape variations are linear maps, so can be represented as 3 x 3
///   matrices.
///   The matrices of the 5 basis elements are stored here.
///   The first index is the number of the basis element.
...
void
ew::Tps3::uniform_basis(double *basis, double *transforms_opt,
 ...
 const double *lmks, int n_lmks)
{
  ...
  double c01 = std::sqrt(sum_squares[0] / sum_squares[1]);
  double c12 = std::sqrt(sum_squares[1] / sum_squares[2]);
  double c20 = std::sqrt(sum_squares[2] / sum_squares[0]);
  double sc01 = std::sqrt((sum_squares[0] + sum_squares[1] + sum_squares[2]) /
   (sum_squares[0] + sum_squares[1]));
  double sc12 = std::sqrt((sum_squares[0] + sum_squares[1] + sum_squares[2]) /
   (sum_squares[1] + sum_squares[2]));
  double sc20 = std::sqrt((sum_squares[0] + sum_squares[1] + sum_squares[2]) /
   (sum_squares[2] + sum_squares[0]));
  double sd0 = std::sqrt(1.0 /
   (sum_squares[0] * (sum_squares[1] + sum_squares[2])));
  double sd12 = std::sqrt((sum_squares[0] + sum_squares[1] + sum_squares[2]) /
   (sum_squares[1] * sum_squares[2] * (sum_squares[1] + sum_squares[2])));
  for (int sh = 0; sh < 5; sh += 1) {
    ...
    case 2:
      ...
      break;
    case 3:
      m[0] = sd0 * (sum_squares[1] + sum_squares[2]);
      m[4] = -sd0 * sum_squares[0];
      m[8] = -sd0 * sum_squares[0];
      break;
    case 4:
      m[0] = 0.0;
      m[4] = sd12 * sum_squares[2];
      m[8] = -sd12 * sum_squares[1];
      break;
    }
    double mr[D * D];
    for (int i = 0; i < D; i += 1) {
    ...
    for (int i = 0; i < n_lmks; i += 1) {
      basis[D * n_lmks * sh + D * i] =
       ...
       rmr[2] * (lmks[D * i + 2] - centroid[2]);
      basis[D * n_lmks * sh + D * i + 1] =
       rmr[3] * (lmks[D * i] - centroid[0]) +
       rmr[4] * (lmks[D * i + 1] - centroid[1]) +
       rmr[5] * (lmks[D * i + 2] - centroid[2]);
      basis[D * n_lmks * sh + D * i + 2] =
       rmr[6] * (lmks[D * i] - centroid[0]) +
       rmr[7] * (lmks[D * i + 1] - centroid[1]) +
       rmr[8] * (lmks[D * i + 2] - centroid[2]);
    }
  }
}
